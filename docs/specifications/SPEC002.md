...
- 快楽系の状態異常が有効な間、拘束からの抵抗が断続的に不利になる状況を作る。その時間帯に抵抗すると、進捗が止まるだけでなく後退する。`穢れ` による既存の脱出困難化とは独立した軸として成立させる。
...
4. 快楽系の状態異常が有効な拘束中に、抵抗すると拘束ゲージが後退する時間帯が断続的に発生し、その時間帯はゲージの色で識別できる。快楽系の状態異常が1つも有効でない拘束中には発生しない。
...
- 窓の内側では、抵抗入力を拘束ゲージへ反映しないだけでなく、**抵抗した分だけゲージを減少させる**。窓の内側で抵抗することは、進捗が止まるのではなく後退する。
- 「抵抗した量」は、拘束ゲージが上昇しようとした量とする。MODは抵抗入力そのものを観測できないため、これが唯一の検出手段である（DEC-114）。
- 減少量は、上昇しようとした量に `NullificationResistPenalty` を乗じた値とする。係数 `1.0` は「得られるはずだった分を逆に失う」ことを意味する。係数 `0` は減少を行わず、上昇を止めるだけの挙動となる。
- 減少後の値は `0` を下限とする。減少後の値は、その窓の内側での新しい上限となる。窓が閉じるまで、失った分を取り戻すことはできない。
- 窓は拘束ゲージの減衰（`DeclineValue` に由来する挙動）を変更しない。減衰による下降は罰とは独立に働き、下降後の値も新しい上限となる。
- 窓の外では、ゲージは素の挙動へ戻る。窓の内側で失った分を窓の外で取り戻すことを妨げない。
- 窓の内側と外側で、拘束ゲージの表示（`GachaGachaSystem` のホールドUI）を隠さない。窓の内側では抵抗するとゲージが後退することが、既存の表示から観測できる状態を保つ。
- 窓の内側では拘束ゲージの塗り色を設定色へ変更する。変更するのは色だけであり、ゲージの値の算出、減衰、表示位置、UIの構造を変更しない。窓の終了時、拘束の終了時、シーン遷移時、巻き戻し時に元の色へ戻す。塗り色を持つ要素を解決できない場合は着色を行わず、窓自体は成立させる。
- 減少罰が有効な間（`NullificationResistPenalty` が `0` より大きい）、着色は `HighlightGauge` の設定にかかわらず行う。罰があるのに合図がない構成は、理由の分からない損失だけを生む（DEC-115）。
...
| `Pleasure` | `NullificationResistPenalty` | float | `1.0` | 窓の内側で抵抗したとき、上昇しようとした量へ乗じて減少させる係数。`0` で減少なし（上昇を止めるだけ）。 |
| `Pleasure` | `HighlightGauge` | bool | `true` | 無力化窓の間、拘束ゲージを着色するか。`NullificationResistPenalty` が `0` より大きい間は、この設定にかかわらず着色する。 |
...
| FR-136 | 窓の内側では、MODは拘束ゲージが上昇しようとした量に `NullificationResistPenalty` を乗じた分だけゲージを減少させなければならない。減少後の値は `0` を下限とし、その窓の内側での新しい上限としなければならない。係数が `0` の場合は減少させず、上昇のみを止めなければならない。 | Must | 抵抗を「無効」ではなく「不利」にする（DEC-114） |
| FR-137 | `NullificationResistPenalty` が `0` より大きい間、MODは `HighlightGauge` の値にかかわらず拘束ゲージを着色しなければならない。 | Must | 罰に対する合図の保証（DEC-115） |
...
| AC-135 | FR-136 | Given 窓が開いている / When 抵抗入力を行う / Then ゲージが上昇せず、抵抗した分に応じて減少する。Given 抵抗をやめる / Then 減衰だけが働き、罰による追加の減少は起きない |
| AC-136 | FR-136 | Given 窓の内側でゲージが `0` 付近まで減った / When さらに抵抗を続ける / Then ゲージは `0` を下回らず、負の値やアンダーフローが発生しない |
| AC-137 | FR-136 | Given 窓の内側で罰により減少した / When 同じ窓の内側で抵抗を続ける / Then 失った分を取り戻せない。Given 窓が閉じる / Then 以降の抵抗は通常どおりゲージを上昇させる |
| AC-138 | FR-136 | Given `NullificationResistPenalty = 0` / When 窓の内側で抵抗する / Then ゲージは上昇も減少もせず、減衰だけが働く |
| AC-139 | FR-137 | Given `HighlightGauge = false` かつ `NullificationResistPenalty > 0` / When 窓が開く / Then ゲージが着色される |
...
| DEC-114 | 「抵抗した量」を、拘束ゲージが上昇しようとした量として検出する。 | 実測により、抵抗入力の反映経路は特定できなかった。`GachaGachaSystem.Execution` へのパッチは適用され窓も開くのに入力が通る（付録A A-7）。経路が分からない以上、入力そのものは観測できない。一方、ゲージが上昇しようとしたことは値を読むだけで分かり、その量は入力の強さに比例する。経路を当てる作業を続けるより、当てずに済む信号を使うほうが、ゲーム更新に対しても頑健である。 | `GachaBind.Execution` など別のメソッドを順に試す、固定量を引く、窓の間だけ減衰を強める |
| DEC-115 | 減少罰が有効な間は、`HighlightGauge` の設定にかかわらず着色する。 | 罰がない旧仕様では、色は「入力が効いていない」ことの feedback にすぎず、消せてよかった。罰があると、色は「今は手を止めろ」という唯一の合図になる。合図を消せる設定を残すと、理由の分からない損失だけが起きる構成を作れてしまう。色の変更自体は引き続き設定できる。 | 設定を尊重して消せるままにする、消せるが起動時に警告する |
...
| A-7 | ~~`GachaGachaSystem.Execution` が抵抗入力1回あたりの反映であるか~~ **解消済み（2026-08-08）。反映経路ではない。** パッチは適用され窓も開くのに抵抗入力が通った。IL2CPPのインライン展開の可能性もあり、経路の特定は行わない | 5.3、DEC-114。介入は経路に依存しない方式へ変更済み |
...
|---|---|---|---|---|---|---|
| CHG-001 | 5.3「窓の内側では、抵抗入力を拘束ゲージへ反映しない」 | 5.3「反映しないだけでなく、抵抗した分だけゲージを減少させる」、FR-136 | 意味変更 | ユーザー決定。抵抗を「無効」にするだけでは、窓の間も連打を続けるのが最適手のままで、色を読む判断がプレイに入らない。後退させることで「ピンクの間は手を止める」が成立する | 窓の性質が「入力の無視」から「入力の罰」へ変わる。脱出難度が上がる。連打し続けるプレイヤーは以前より明確に不利になる | 承認済み |
| CHG-002 | （新規） | 5.3「抵抗した量は、拘束ゲージが上昇しようとした量とする」、DEC-114 | 追加 | 実測により抵抗入力の反映経路を特定できず（付録A A-7）、入力そのものを観測する手段がない。ゲージの上昇試行は値を読むだけで検出でき、入力の強さに比例する | 減少量が連打の強さに自然に比例する。フレームレートに依存しない。経路を当てる必要がなくなり、ゲーム更新に対しても頑健になる | 承認済み |
| CHG-003 | （新規） | `NullificationResistPenalty`（既定 `1.0`）、FR-136 | 追加 | 減少の強さを設定で可変にする。既定 `1.0` は「得られるはずだった分を逆に失う」で最も読み解きやすい。`0` は旧挙動（上昇を止めるだけ）へ戻す逃げ道 | 旧挙動が設定で再現できるため、この改訂は不可逆ではない | 承認済み |
| CHG-004 | 6章「`HighlightGauge` … 無力化窓の間、拘束ゲージを着色するか」（無条件に無効化できた） | FR-137、DEC-115。罰が有効な間は設定にかかわらず着色 | 意味変更 | ユーザー決定。罰がない旧仕様では色は feedback にすぎず消せてよかった。罰があると色は「手を止めろ」という唯一の合図になり、消せる設定は理由の分からない損失だけを生む構成を許してしまう | `HighlightGauge = false` は `NullificationResistPenalty = 0` のときだけ効く。色の変更自体は引き続き可能 | 承認済み |
| CHG-005 | 1.2 目的「抵抗入力が断続的に無力化される状況を作る」、1.3 成功条件4 | 「抵抗が断続的に不利になる」「抵抗するとゲージが後退する時間帯が発生し、色で識別できる」 | 意味変更 | CHG-001 と CHG-004 に合わせた上位記述の同期 | 成功条件の判定に「色で識別できる」が加わる | 承認済み |
