...
**このビルドはプローブ段階である。** 快楽・絶頂・堕落・`BreastSuper` はすべて無変更相当の既定値で停止している。プローブは付録Aの各項目を1回ずつログへ記録する。

**v1.1 の改訂を反映済み。** v1.0 の「拘束中のHP0抑止」（旧 A-1）は撤回された。撤回済みの機構と、そのために測った項目は [`SPEC003-withdrawn.md`](../specifications/SPEC003-withdrawn.md) にあり、**検証対象ではない**。v1.1 で新たに実機確認を要するのは A-50（性的被弾のHP減算を止められるか）と A-51（HPを `0` にできるか）である。

付録A のうち **A-5、A-7 は静的調査で解消済み**であり、実機確認を要しない。残るのは以下である。
...
```
SiNiSistar2 Pleasure loaded; suppressSexualHpDamage=True, gauge=off, corruption=off,
climaxGameOver=False, breastSuper=off, probe=True, patches=N.
```

既定では `gauge=off` のため、性的被弾のHP減算の抑止も行われない（FR-278）。この段階でゲームの挙動は何も変わらない。旧 `suppressHp0` は v1.1 で無くなった。
...
### 3.1 A-50／A-51 — 性的ダメージ無効化と致死化（最重要）

**前提。** `PleasureGainPerHit` を正の値にし、`SuppressSexualHpDamage=True` にする。ゲージが止まっている間は抑止も行われないため、既定値のままではこの節を測定できない（FR-278）。

#### 3.1.1 A-50 — 性的被弾がHPを削らないか

| 手順 | 期待結果 | 判定 |
|---|---|---|
| 1. 性的な攻撃を行う敵に拘束され、HPの数値を記録する | 被弾しても**HPが1点も減らない** | **AC-202**。減れば A-50 の手段が効いていない |
| 2. ダメージ表示、被弾演出、状態異常の付与を確認する | 従来どおり発生する | **AC-202**。消えていれば FR-205 違反 |
| 3. 快楽ゲージを確認する | 被弾ごとに上昇する | 上がらなければ被弾が観測できていない（A-2） |
| 4. 脱出し、通常の敵に攻撃される | HPが通常どおり減る | **AC-203**。減らなければ抑止が残っている（FR-204違反、最も危険な失敗） |
| 5. 拘束中に噛みつきなど非性的な攻撃を受ける（宣言 `Auto` の敵） | HPが減る | **AC-240** |
| 6. 毒を受けた状態で性的な敵に拘束され、HPが尽きるまで待つ | HPが `0` に達し、絶頂回数にかかわらずゲームオーバーになる | **AC-239** |

HPが減ってしまう場合、`DontSub` がこのビルドで減算を止めていない。5.1.2の手段2（減少分の即時復元）へ切り替える（FR-275）。

#### 3.1.2 A-51 — 限界到達でHPを `0` にできるか

**前提。** `ClimaxLimitBase` を小さな値（例: `2`）にし、`EnableClimaxGameOver=True` にする。

| 手順 | 期待結果 | 判定 |
|---|---|---|
| 1. 限界の1つ手前まで絶頂させられる。HPは満量のまま | HPが満量のまま絶頂回数だけが増える | 前提の確認 |
| 2. さらに1回絶頂する | **その瞬間にHPが `0` になる**。敵の次の攻撃を待たない | **AC-213** |
| 3. ゲームオーバーの進行を確認する | 死亡演出、リトライ、ペナルティが従来どおり動作する | **AC-213**。止まる場合はHPを `0` にする手段が経路の途中を飛ばしている |
| 4. SPEC001 を同時に導入した構成でログを確認する | `game-over` トリガーが発生する | **AC-213**、A-5 |
| 5. 敗北演出の間を観察する | 致死化が繰り返されない | **AC-243** |
| 6. `EnableClimaxGameOver=False` で限界を超える | HPは `0` にならず、超えた事実がログに出る | **AC-244** |
| 7. 限界以上の状態でセーブし、再ロードして歩き回る | HPは `0` にならない。次の絶頂で `0` になる | **AC-229** |

手順2でHPが `0` にならない場合、5.5.3の順で次の手段（`SetCurrentValue` → `RequestCommonDead`）へ移る。どれも効かない場合は `SuppressSexualHpDamage=False` として従来の敗北条件へ戻す（14.3）。
...
1. カタログファイルを削除し、起動する。起動ログに次が出ることを確認する。件数は v1.2 で両列挙体を投入するため約216件になる（`None` は除外される）。

```
Enemy catalogue: 216 enemies, 1 forced sexual, 0 forced non-sexual, at '...\enemy-attacks.json'.
```
...
**AC-245／A-53 — ギャラリー未登録の敵を名指せる（v1.2、この不具合の再現手順）**

対象は `Dungeon_Sewage7` の**大口のワーム**。v1.1 では拘束相手が `None` と解決され、`GaID_MeatWorm1` 行を `Sexual` にしても何も起きなかった。

1. 快楽が上昇する設定で起動し、大口のワームに拘束される。
2. `[probe]` 行を確認する。`A-53: captor 'EnmID_MeatWorm1' was identified by its EnemyID; the game calls it '大口のワーム'` のように、**解決に使われた欄と表示名**が出る。`identified by its object name` と出た場合は識別子が `obj:EnemyMeatWorm1` になる（どちらでも以降の手順は同じ）。
3. 拘束されたまま `F10` を押す。上部に `Holding you now: 大口のワーム  (EnmID_MeatWorm1)` が出て、その行が先頭付近で選択されていることを確認する。
4. `Space` で `Sexual` にし、`Enter` で保存する。**拘束を続けたまま**次の被弾から快楽が上昇することを確認する。
5. 触手叢（`EnemyMeatTentacleCluster1`）と肉胎児（`EnemyMeatWombBaby`）でも同様に、**それぞれ別の行**として現れることを確認する。片方を `NonSexual` にしても他方の判別が変わらないこと（AC-246）。

**AC-249／A-53 — `EnemyObject` でない拘束者（v1.2、レビュー指摘分）**

`Bind.BinderEnemy` の型は `EnemyObject` であり、`ParasiteTentacle`・`ParasiteBullet`・`StoneEye` はそれを継承せずに拘束する。腸の天井（`MeatCeiling`）のように `EnemyObject` である敵は順1か順2で解決されるはずなので、両方を確認する。

1. 寄生触手、石の眼（大聖堂の絵画ギャラリー）、腸の天井のそれぞれに拘束される。
2. 拘束のたびに `[probe] A-53:` の行が1本出ることを確認する。**1本も出ない拘束があれば、その場所と敵を4章へ記入する**（拘束相手を両経路とも取得できていない＝順4に落ちている）。
3. `F10` を開き、上部が `(not held)` になっていないこと、その拘束者の行が存在することを確認する。`Sexual` にして保存し、次の被弾から快楽が上昇することを確認する。

**AC-246 — `None` 行が消える**

1. v1.1 で作られたカタログを用意する（`"id": "None"` の行がある）。
2. 起動する。カタログを開き、`None` の行が**無くなっている**ことを確認する。
3. その行の `kind` が `Auto` 以外だった場合、起動ログに引き継げなかった旨の警告が1回出ることを確認する。

**AC-247 — 表示名（A-54）**

1. `F10` を開く。拘束された経験のある行に、識別子とゲーム内表記名が併記されていることを確認する。
2. カタログファイルに `"displayName": "大口のワーム"` が**エスケープされずそのまま**書かれていることを確認する。
3. ゲームの言語を切り替え、同じ敵に拘束される。表示名が新しい言語のものへ書き換わることを確認する。
4. 表示名が出ない敵があれば、その識別子を4章へ記入する（A-54）。

**AC-248 — 既存の宣言が生きている**

1. v1.1 で `GaID_Mimic` を `Sexual` にしたカタログのまま v1.2 で起動する。
2. ミミックに拘束され、快楽が上昇することを確認する。カタログの `GaID_Mimic` 行が改名も再作成もされていないことを確認する。
...
3. **1回の使用につき計数が1だけ進むこと**を確認する。4つの経路すべてにpostfixを当てているため、同じ付与が複数回報告される可能性があるが、フレーム単位で1回に畳んである。2以上進む場合は畳み込みが効いていない。
4. 3回目で `Breast escalated to BreastSuper` が出ることを確認する。
5. `[probe] A-15:` で、アイテムがどの経路を通り、実際に何を付与したかを記録する。行は付与先がプレイヤーでなくても出る。**1行も出ない場合のみ**、どの経路も観測できていないことになる。

**A-16 — 停止中の付与**

アイテム画面を開いている間、ゲームは停止する。`A-15` の各行に `timeScale` が付くので、停止中に付与が起きているかが分かる。

- `timeScale 0` で付与が起きている場合、遷移は**画面を閉じてから**成立する。`[probe] The BreastSuper escalation is waiting for the game to resume` が出る。描画済みのメニューの背後で状態異常を足さないためであり、意図した挙動である。
- `timeScale 1` なら停止はタイムスケール以外の手段によるもので、遷移は即座に成立する。
