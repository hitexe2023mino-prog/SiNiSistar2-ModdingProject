...
このディレクトリを、リポジトリ固有のエージェントスキルの正本とする。作業に該当するスキルがある場合は、対象の `SKILL.md` を最後まで読んでから実行する。参照資料は各 `SKILL.md` に記載された条件に従って必要なものだけを読む。
...
検証は、配置、frontmatter、命名、TODO、本文長、相対リンク、UIメタデータを確認する。内容面では、上の責務境界と、対象スキルのワークフローが一致しているかも差分レビューする。
